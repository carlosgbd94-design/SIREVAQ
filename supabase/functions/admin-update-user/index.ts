...
serve(async (req) => {
  ...
  try {
    ...
    const { usuario: internalID, rol, municipio, clues, unidad } = payload;

    if (!internalID) {
      throw new Error('El ID de usuario es obligatorio');
    }
    if (!rol || !ROLES_VALIDOS.includes(String(rol).toUpperCase())) {
      throw new Error('El rol proporcionado no es válido');
    }

    // Solo se reasigna la ubicación institucional (rol/municipio/unidad/CLUES).
    // El identificador de usuario, el correo de acceso y el historial de reportes
    // capturados (que guardan su propio CLUES/municipio/unidad al momento de la
    // captura) no se ven afectados por este cambio.
    const updateData = {
